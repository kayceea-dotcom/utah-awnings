...
function MobilePriceBar({ result, onExpand }: { result: ReturnType<typeof calcWPan>; onExpand: () => void }) {
...
function PriceSummaryPanel({ result, onClose }: { result: ReturnType<typeof calcWPan>; onClose?: () => void }) {
  const fmt = (n: number) => n.toLocaleString("en-US", { style: "currency", currency: "USD" });
  const margin = result.totalJobSale > 0 ? result.totalProfit / result.totalJobSale : 0;
  return (
...
export default function WPanQuotePage() {
  ...
  useEffect(() => {
    ...
    if (inp.jogType === "deck" && hasRun2 && inp.downspouts < 2) {
      setInp((p) => ({ ...p, downspouts: 2 }));
    }
  }, [inp.jogType, inp.projection2, inp.width2, inp.downspouts]);

  function setField<K extends keyof WPanInputs>(key: K, val: WPanInputs[K]) {
    setInp((p) => ({ ...p, [key]: val }));
  }
          ...
          <div className="flex gap-6 items-start">
            <div className="flex-1 min-w-0 space-y-3">
              ...
              <SectionCard id="colors" title="Colors" open={open.has("colors")} onToggle={toggleSection}>
                <SelectInput label="Pan Color" value={inp.colorPans} onChange={(v) => setField("colorPans", v)} options={COLOR_OPTS} />
                <SelectInput label="Gutter / Fascia" value={inp.colorGutterFascia} onChange={(v) => setField("colorGutterFascia", v)} options={COLOR_OPTS} />
