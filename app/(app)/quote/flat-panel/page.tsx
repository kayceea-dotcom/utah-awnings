...
function SectionCard({
  ...
  return (
    <div className="card overflow-hidden">
      ...
      {open && (
        <div className="px-4 lg:px-5 pb-5 pt-1 grid grid-cols-2 gap-3 lg:gap-4">
          {children}
        </div>
      )}
    </div>
...
function PriceSummaryPanel({ result, onClose }: {
  result: ReturnType<typeof calcNewport>;
  onClose?: () => void;
}) {
  const fmt = (n: number) => n.toLocaleString("en-US", { style: "currency", currency: "USD" });
          ...
          <div className="flex gap-6 items-start">
            ...
            <div className="flex-1 min-w-0 space-y-3">
              ...
              <SectionCard id="dimensions" title="Dimensions" open={open.has("dimensions")} onToggle={toggleSection}>
                ...
                <SelectInput label="Jog Type" value={inp.jogType} onChange={(v) => setField("jogType", v as never)}
              ...
              {/* Mobile diagrams - the desktop right panel below is hidden on
                  mobile, so it's not enough on its own the way it is for the
                  price summary/commission panels (which have their own mobile
                  bottom-bar equivalent). Always 2D here regardless of the 3D
                  viewer toggle - that toggle and its WebGL panel are desktop-
                  only, same as Pergola's mobile diagrams. */}
