...
export default function PergolaQuotePage() {
  ...
  function handleMountStyleChange(v: string) {
    if (v === "freestanding" && inp.mountStyle !== "freestanding") {
      const { front, rear } = splitPostsEvenly(inp.posts + inp.rearPosts);
  ...
  function handleTotalPostsChange(v: number) {
    ...
    setInp((p) => ({ ...p, posts: front, rearPosts: rear }));
  }
          ...
          <div className="flex gap-6 items-start">
            <div className="flex-1 min-w-0 space-y-3">
              ...
              <SectionCard id="posts" title="Posts" open={open.has("posts")} onToggle={toggleSection}>
                <NumInput label={inp.mountStyle === "freestanding" ? "Posts (Total, qty)" : "Posts (qty)"}
                  value={inp.mountStyle === "freestanding" ? inp.posts + inp.rearPosts : inp.posts}
                  onChange={inp.mountStyle === "freestanding" ? handleTotalPostsChange : (v) => setField("posts", v)}
                  hint={inp.mountStyle === "freestanding" ? "Splits evenly - front beam + rear beam" : undefined} />
                <SelectInput label={inp.mountStyle === "freestanding" ? "Front Post Height (ft)" : "Post Height (ft)"} value={String(inp.postHeight)} onChange={(v) => setField("postHeight", Number(v))}
